...
def train_GCN(x_test, x_train,lr, weight_decay,patience,n_epochs,batchsize,dataname):
    ...
    for epoch in range(n_epochs): 
        traindata_list, testdata_list = loadData(dataname, x_train, x_test, droprate=0.2) #0.4
        ...
        for Batch_data in tqdm_train_loader:
            ...
            out_labels, cl_loss, y = model(Batch_data) 
            finalloss = F.nll_loss(out_labels,y)
            loss_adv = finalloss + 0.001*cl_loss
            loss_adv.backward()
            fgm.restore()
            optimizer.step()
            ##--------------------------------##
...
data_path =  './data/twitter16/' # ./data/twitter15/ ./data/pheme/
